...
class SelectKGroups_Attn(nn.Module):
    def __init__(self, num_group, top_k_group, hidden_size, resnet_num, config, attn_hid_dim):
        super(SelectKGroups_Attn, self).__init__()
        # 初始化可训练的选择权重矩阵，大小为 (k_num_group, num_group)
        # self.selection_weights = nn.Parameter(torch.rand(top_k_group, num_group))
    
        self.down_proj = nn.Linear(hidden_size, attn_hid_dim, bias=False)

        self.up_proj = nn.Linear(attn_hid_dim, hidden_size, bias=False)
        ...
        self.r = RouterModel(config, attn_hid_dim)
    ...
    def forward(self, 
        ...
        all_draft_hidden_states = self.resnet_block(all_draft_hidden_states).to(all_draft_hidden_states.dtype)

        _, bsz, seq_len, hid_dim = all_draft_hidden_states.shape

        # 通过矩阵乘法选择k_num_group
        # 首先将 all_hidden_states 转置为 (1, bs*seqlen, hidden_dim, num_group)
        all_draft_hidden_states = all_draft_hidden_states.permute(1, 2, 3, 0).contiguous()
        
        # 使用 selection_weights 从 num_group 中选择 k_num_group
        # selected_output = torch.matmul(all_hidden_states, self.selection_weights.transpose(0, 1)).to(all_hidden_states.dtype)
        selected_output = self.selection_layer(all_draft_hidden_states).permute(3, 0, 1, 2).contiguous().to(all_draft_hidden_states.dtype)
        # selected_output = self.router_down_proj(self.router_act(self.router_up_proj(all_hidden_states))).to(all_hidden_states.dtype)

        selected_output = self.down_proj(selected_output)

        topk = selected_output.shape[0]

        final_output = []

        for i in range(topk):
            selected_output_i = self.r(
                    selected_output[i],
                    attention_mask,
                    position_ids,
                    past_key_values,
            ...
            final_output.append(selected_output_i)
        
        final_output = torch.stack(final_output, dim=0)

        final_output = self.up_proj(final_output)
        
        # 输出形状为 (bs*seqlen, hidden_dim, k_num_group)
        return final_output

def add_router_attn(self, attn_hid_dim):

    self.router = SelectKGroups_Attn(self.num_hidden_layers - self.top_layers_len, self.top_k_group, self.hidden_size, self.resnet_num, self.config, attn_hid_dim)

    self.router.to(self.dtype).to(self.device)


class SelectKGroups_FC(nn.Module):
    def __init__(self, num_group, top_k_group, hidden_size, resnet_num):
        super(SelectKGroups_FC, self).__init__()
        # 初始化可训练的选择权重矩阵，大小为 (k_num_group, num_group)
        # self.selection_weights = nn.Parameter(torch.rand(top_k_group, num_group))
    ...
    def forward(self, all_hidden_states, base_hidden_states):
        
        all_hidden_states = self.resnet_block(all_hidden_states).to(all_hidden_states.dtype)

        selected_output = all_hidden_states
        ...
        # 使用 selection_weights 从 num_group 中选择 k_num_group
        # selected_output = torch.matmul(all_hidden_states, self.selection_weights.transpose(0, 1)).to(all_hidden_states.dtype)
        # selected_output = self.selection_layer(all_hidden_states).to(all_hidden_states.dtype).permute(3, 0, 1, 2).contiguous()
        # selected_output = self.router_down_proj(self.router_act(self.router_up_proj(all_hidden_states))).to(all_hidden_states.dtype)

        base_hidden_states = base_hidden_states.unsqueeze(0)
        base_hidden_states = base_hidden_states.expand(selected_output.shape[0], -1, -1, -1)

        selected_output = torch.cat([selected_output, base_hidden_states], dim=-1)
        selected_output = self.fc(selected_output).to(selected_output.dtype)

        # 输出形状为 (bs*seqlen, hidden_dim, k_num_group)
        return selected_output

def add_router_fc(self):

    self.router = SelectKGroups_FC(self.num_hidden_layers - self.top_layers_len, self.top_k_group, self.hidden_size, self.resnet_num)
...
def replace_compute_loss_kl_div(
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        ee_logits = ee_outputs[1]
        ee_loss = ee_outputs[0]

        base_logits = origin_outputs[0]
        
        loss_function = nn.KLDivLoss(reduction="batchmean")
        loss_logits = (
            loss_function(
                F.log_softmax(ee_logits / distill_temperature, dim=-1),
                F.softmax(base_logits / distill_temperature, dim=-1),
            )
            * (distill_temperature ** 2)
        )

        loss = distill_alpha * ee_loss + (1.0 - distill_alpha) * loss_logits

        log[f"ee_loss"] = loss.item()
        ...
        return (loss, ee_logits) if return_outputs else loss
...
def replace_compute_loss_cross_entropy_base(
    ssd_groups_coefficient,
    ssd_decay_coefficient, 
    ssd_scheduler="constant",
    ssd_logging=False,
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        loss_mask = inputs['labels'].ne(IGNORE_TOKEN_ID).int()
        ...
        for i in range(num_group):
            ...
            if i == 0:
                base_p = nn.Softmax(dim=-1)(all_logits[i])
                base_p = base_p.detach()
            else:
                draft_logp_i = nn.LogSoftmax(dim=-1)(all_logits[i])
                plogp_i = base_p * draft_logp_i

                loss_i = -torch.sum(torch.sum(loss_mask * plogp_i, -1)) / loss_mask.sum()
