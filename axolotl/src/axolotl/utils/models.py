...
from axolotl.monkeypatch.ssd_utils import (
    replace_compute_loss_cross_entropy,
    replace_compute_loss_cross_entropy_base,
    replace_compute_loss_kl_div,
...
def load_model(
    ...
    try:
        if cfg.is_llama_derived_model and not cfg.trust_remote_code and not cfg.gptq:
            ...
            if cfg.ssd_layer_group is not None:
                from .modeling_llama_ssd_v1_top_layers_single import LlamaForCausalLM
                config_kwargs["ssd_layer_group"] = cfg.ssd_layer_group
            elif cfg.ssd_layer_groups is not None:
                ...
                if cfg.davm:
                    ...
                    if cfg.fc:
                        from .modeling_llama_ssd_v1_top_layers_router_fc import LlamaForCausalLM
                    elif cfg.attn:
                        from .modeling_llama_ssd_v1_top_layers_router_last_attn import LlamaForCausalLM
                    else:
                        from .modeling_llama_ssd_v1_top_layers_router_last import LlamaForCausalLM
                else:
            ...
            config_kwargs["top_layers_len"] = cfg.top_layers_len
    ...
    if cfg.top_k_group is not None:
        ...
        if cfg.fc:
            ...
            add_router_attn(model, cfg.attn_hid_dim)
        else:
            add_router(model)
        ...
        if cfg.adapter in ["lora", "qlora"]:
            ...
            if cfg.lora_modules_to_save is None:
                ...
                cfg.lora_modules_to_save.append(f"router")
    ...
    # [MODIFIED] Add support for SSD (https://github.com/FasterDecoding/Medusa)
    elif cfg.ssd_layer_group is not None or cfg.ssd_layer_groups is not None:

        if cfg.ssd_layer_group:
            from .modeling_llama_ssd_v1_top_layers_single import LlamaForCausalLM
        elif cfg.ssd_layer_groups:
            from .modeling_llama_ssd_v1_top_layers import LlamaForCausalLM
            # from .modeling_llama_ssd_v1_top_layers_router import LlamaForCausalLM
        ...
        if cfg.ssd_layer_group is not None:
            ...
            LOG.info(
                f"using draft group {cfg.ssd_layer_group}, with top {cfg.top_layers_len} kept the same, {cfg.ssd_logging} logging"
            )
            ...
            LOG.info(
                f"using draft groups {cfg.ssd_layer_groups}, with top {cfg.top_layers_len} kept the same, {cfg.ssd_logging} logging"
            )
    ...
    if cfg.top_k_group and cfg.adapter:
