...
from transformers import top_k_top_p_filtering, AutoConfig
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if draft_attn_skip_mask and (self.layer_idx not in draft_attn_skip_mask):
        ...
        if draft_mlp_skip_mask and (self.layer_idx not in draft_mlp_skip_mask):
            hidden_states = residual
        else:
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                def create_custom_forward(module):
                    def custom_forward(*inputs):
                        # None for past_key_value
                        return module(*inputs, past_key_value, output_attentions, padding_mask=padding_mask, draft_attn_skip_mask=draft_attn_skip_masks, draft_mlp_skip_mask=draft_mlp_skip_masks)
                ...
                layer_outputs = decoder_layer(
                    ...
                    use_cache=use_cache,
                    padding_mask=padding_mask,
                    draft_attn_skip_mask=draft_attn_skip_masks,
                    draft_mlp_skip_mask=draft_mlp_skip_masks,
                )
        ...
        if not return_dict:
            return tuple(v for v in [hidden_states, base_hidden_states, next_cache, all_hidden_states, all_self_attns] if v is not None)
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.ssd_layer_group = config.ssd_layer_group
    ...
    def forward(
        ...
        outputs = self.model(
            ...
            draft_attn_skip_masks=self.ssd_layer_group,
            draft_mlp_skip_masks=self.ssd_layer_group,
            top_layers_len=self.top_layers_len,
        )

        draft_hidden_states = outputs[0]
        base_hidden_states = outputs[1]
        if self.config.pretraining_tp > 1:
            lm_head_slices = self.lm_head.weight.split(self.vocab_size // self.config.pretraining_tp, dim=0)
            draft_logits = [F.linear(draft_hidden_states, lm_head_slices[i]) for i in range(self.config.pretraining_tp)]
        ...
        if not return_dict:
            output = (draft_logits, base_logits) + outputs[2:]
