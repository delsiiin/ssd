...
class RouterModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        if past_key_values is not None:
            if past_key_values[l_idx] is not None:
                past_key_values_length = past_key_values[l_idx][0].shape[2]
        ...
        past_key_value = past_key_values[l_idx] if past_key_values is not None else None
        ...
        if self.gradient_checkpointing and self.training:
            ...
            layer_outputs = torch.utils.checkpoint.checkpoint(
                create_custom_forward(self.layers[l_idx]), hidden_states, attention_mask, position_ids
            )

        else:

            layer_outputs = self.layers[l_idx](
                hidden_states,
                attention_mask=attention_mask,
                position_ids=position_ids,
                past_key_value=past_key_value,
