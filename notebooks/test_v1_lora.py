...
if __name__ == '__main__':
    ...
    try:
        with open('optimal_solution.json', 'r') as json_file:
            ...
            # 将 attn 和 mlp 解转换为 skip masks 的形式
            draft_attn_skip_masks = [layers for group, layers in attn_solution.items()]
            draft_mlp_skip_masks = [layers for group, layers in mlp_solution.items()]

            print("Draft attn skip masks:")
            print(draft_attn_skip_masks)

            print("Draft mlp skip masks:")
            print(draft_mlp_skip_masks)

    except FileNotFoundError:
        print("No solution file found.")

    # <<< same attn and mlp skip masks >>>
    # draft_attn_skip_masks = [[3, 6, 7, 12, 13, 22, 23, 25, 26, 27], [0, 1, 2, 5, 9, 10, 17, 18, 19, 30], [4, 8, 11, 14, 15, 16, 21, 24, 28, 31]]
    # draft_mlp_skip_masks = [[3, 6, 7, 12, 13, 22, 23, 25, 26, 27], [0, 1, 2, 5, 9, 10, 17, 18, 19, 30], [4, 8, 11, 14, 15, 16, 21, 24, 28, 31]]

    # <<< different attn and mlp skip masks (unsorted) >>>
    # draft_attn_skip_masks = [[5, 10, 12, 13, 15, 19, 21, 23, 25, 28], [1, 2, 3, 4, 16, 17, 18, 20, 22, 26], [0, 6, 7, 8, 9, 11, 14, 24, 29, 31]]
    # draft_mlp_skip_masks = [[3, 6, 7, 12, 13, 22, 23, 25, 26, 27], [0, 1, 2, 5, 9, 10, 17, 18, 19, 30], [4, 8, 11, 14, 15, 16, 21, 24, 28, 31]]

    # <<< different attn and mlp skip masks (sorted) >>>
    # draft_attn_skip_masks = [[3, 4, 6, 11, 12, 13, 16, 20, 22, 24, 26, 31], [0, 1, 2, 5, 8, 17, 18, 19, 21, 23, 25, 27]]
    # draft_mlp_skip_masks = [[6, 7, 9, 12, 13, 17, 21, 23, 24, 25, 27, 28], [0, 1, 2, 3, 4, 8, 11, 14, 18, 22, 26, 31]]

    # <<< Draft & Verify >>>
    # draft_attn_skip_masks = [[4, 7, 9, 10, 17, 20, 22, 23, 24, 25, 27, 28, 29, 30]]
    # draft_mlp_skip_masks = [[2, 6, 7, 8, 10, 15, 23, 25, 29]]

    # <<< different attn and mlp skip masks (unsorted) (all prompts) (cnn_dailymail& xsum) >>>
    # draft_attn_skip_masks = [[2, 4, 7, 10, 14, 18, 22, 23, 28, 30], [3, 5, 8, 11, 13, 15, 17, 24, 27, 31], [0, 1, 6, 9, 12, 16, 19, 20, 21, 25]]
    # draft_mlp_skip_masks = [[1, 5, 8, 13, 15, 17, 20, 21, 25, 30], [2, 4, 7, 12, 14, 16, 18, 19, 23, 28], [0, 3, 9, 10, 11, 22, 24, 27, 29, 31]]

    # draft_attn_skip_masks = [[3, 4, 5, 10, 11, 24, 26, 30], [2, 9, 12, 15, 17, 18, 25, 27], [0, 1, 8, 14, 20, 21, 23, 28], [6, 7, 13, 16, 19, 22, 29, 31]]
    # draft_mlp_skip_masks = [[4, 8, 13, 14, 16, 22, 23, 31], [2, 5, 9, 11, 17, 20, 26, 28], [6, 7, 19, 21, 25, 27, 29, 30], [0, 1, 3, 10, 12, 15, 18, 24]]
    ...
    lora_path = "/root/idea/speculative_decoding/Medusa/axolotl/vicuna-7b-v1.3-qlora-ssd-out-groups_0"
    ...
    for idx,x in tqdm(enumerate(data)):
        ...
        with torch.inference_mode():
            ...
            with model.self_draft():
                # print(draft_attn_skip_masks)
                draft_logits, base_outputs, base_logits = model(input_ids, output_orig = True, past_key_values=model.past_key_values, draft_attn_skip_masks=draft_attn_skip_masks, draft_mlp_skip_masks=draft_mlp_skip_masks, top_layers_len=args.top_layers_len)
            inference_count += 1
            ...
            for _ in range(args.max_new_tokens):
                with model.self_draft():
                    draft_logits, base_outputs, base_logits = model(preds.cuda().unsqueeze(0), output_orig = True, past_key_values = model.past_key_values, draft_attn_skip_masks=draft_attn_skip_masks, draft_mlp_skip_masks=draft_mlp_skip_masks, top_layers_len=args.top_layers_len)
                inference_count += 1
