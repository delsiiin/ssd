...
"""
    The draft models will copy the past_key_values from verify model before the drafting process (w/o using full model to generate 1st token).
    No overlaps are allowed between draft layers -- draft_attn_skip_mask and draft_mlp_skip_mask.
    Each draft layer group generates one token at a step.
    Need a warm up.
"""
...
from .router_tree import RouterModel

import copy

# Copied from Draft&Verify codes
##########################################################################################################################
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if idx >= top_layers_len:
                d_hidden_states = self.norm(hidden_states)
                draft_hidden_states.append(d_hidden_states)
...
def add_router(self, attn_hid_dim):
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    # Copied from Draft&Verify
    @contextmanager
    def self_draft(self, enabled: bool = True):
        """
        Context manager to enable or disable Draft&Verify for the model. This context manager is intended to be used
        during inference to enable Draft&Verify for the model.

        Args:
            enabled (`bool`, *optional*, defaults to `True`):
                Whether to enable Draft&Verify for the model.
        """
    ...
    def forward(
        ...
        all_draft_hidden_states = self.router(
                all_draft_hidden_states=all_draft_hidden_states,
                attention_mask=attention_mask,
                position_ids=position_ids,
        ...
        if not return_dict:
            output = (all_draft_logits, base_logits)
